...
class homeController {
  async show(req, res, next) {
    ...
    const { flashDealProduct, hotProduct, allProduct } = products.reduce(
      (acc, item, index) => {
        if (item.status === 'flash-sale' && acc.flashDealProduct.length < 5) acc.flashDealProduct.push(item)
        if (item.status === 'hot' && acc.hotProduct.length < 5) acc.hotProduct.push(item)
